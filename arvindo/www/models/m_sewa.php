...
class m_sewa extends CI_Model {
    ...
    function table(){
        ...
        $table->clear();
        $table->detail(site_url('sewa/sewa_dtl'));
        $table->detail_tipe('detil_priview_bottom');
        $table->orderby('kdlokasi');
        $table->sortby('DESC');
        $table->action(site_url('sewa/getTables'));
        ...
        #LIST ARRAY PROCESS
        $process['Tambah Sewa'] = array('GETHASH', 'sewa/form', '1', 'f' . $type . '_list');
    ...
    function table_detail($id){
        $table = $this->newtable;
        $type = 'dataDokumen';
        $query = "SELECT A.idijin, tgl_ijin 'TANGGAL IJIN', tgl_akhir 'TANGGAL AKHIR', nominal 'NOMINAL', CONCAT('<a href=', B.fileupload ,' target=\'_blank\'>Preview File Ijin', '</a>') '' 
                  FROM m_lokasi_sewa A
                  LEFT JOIN (SELECT fileupload, idijin FROM m_lokasi_sewa_file ORDER BY date_create DESC ) B ON B.idijin = A.idijin 
                  WHERE kdlokasi = '".$id."'";
        ...
        #LIST ARRAY PROCESS
        $process['Tambah Ijin'] = array('GETHASH', 'sewa/form', '1', 'f' . $type . '_list');

...
class Excel extends PHPExcel {
    ...
    function applayFormat($cell, $format) {
        foreach ($cell as $head) {
            PHPExcel::getActiveSheet()->getStyle($head)->applyFromArray($format);
        }
        return;
    }
    ...
    function mergecell($merge = NULL, $center = TRUE) {
        foreach ($merge as $start => $end) {
            ...
            if ($center)
                PHPExcel::getActiveSheet()->getStyle($end[0] . ':' . $end[1])->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        }
        return;
    }
